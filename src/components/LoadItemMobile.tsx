...
const LoadItemMobile = ({
  ...
  return (
    <div className="mb-2 dark:mb-0" onClick={handeOpenComment}>
      ...
      >
        <div
          className="length text-[12px] absolute  bottom-[22px] right-4  border-[#2183d9] z-50 border-[0.7px]  text-blue-500 dark:text-gray-300 font-bold  px-2 py-2  rounded-xl lg:hidden block"
          onClick={(e) => {
            e.stopPropagation();
          }}
        >
        ...
        {boardType === "load" && (
          ...
          to={`/dashboard/preview/${id}`}
            className="length  text-[12px] absolute  bottom-[22px] left-4 border-[#2183d9]  z-50 border-[0.7px] text-black dark:text-white px-10 py-2  rounded-xl lg:hidden flex justify-center items-center gap-4 tracking-wider"
            onClick={(e) => {
              e.stopPropagation();
            }}
          >
